...
const main = ({ rootDirectory }) => {
  ...
  const getHostedZoneIdByName = () => __async(void 0, null, function* () {
    let finished = false;
    let props = {};
    while (!finished) {
      const {
        ...
        IsTruncated,
        NextMarker
      } = yield route53.listHostedZones(props).promise();
...
aborting...`);
    ...
    },
    {
      title: "Set up Clerk",
      task: () => {
        return rlp(`Create an application on https://dashboard.clerk.dev/applications called ${projectName}. Press enter when done.`).then(() => rlp("Enter the developer api key:").then((k) => process.env.CLERK_DEV_API_KEY = k)).then(() => rlp("Enter the developer clerk frontend API url:").then((k) => process.env.CLERK_DEV_FRONTEND_API = k)).then(() => console.log(import_chalk.default.blue("Check on custom urls in redirect config. Then create production instance on same settings.\nCurrently, there's a Clerk bug where you have to duplicate this work in production."))).then(() => rlp("Enter the production api key:").then((k) => process.env.CLERK_API_KEY = k)).then(() => rlp("Enter the clerk production id, found on the DNS page:").then((k) => {
    ...
    },
    {
      ...
      task: () => {
        const view = {
          ...
          DomainName,
          mysqlName,
          displayName: projectName.split(".").map((s) => `${s.slice(0, 1).toUpperCase()}${s.slice(1)}`)[0],
...
`);
    ...
    },
    {
      ...
      task: () => {
        ...
        const userTfOpts = __spreadProps(__spreadValues({}, tfOpts), {
          headers: __spreadProps(__spreadValues({}, tfOpts.headers), {
        ...
        return import_axios.default.get("https://app.terraform.io/api/v2/organizations/VargasArts/oauth-clients", tfOpts).then((r) => {
          ...
          if (!inEnv && !d.value) {
            console.log(import_chalk.default.yellow("Did not find value or env", d.env, "for key", d.key, "skipping..."));
            return false;
          }
          return true;
        }).map(({ key, env, value }) => import_axios.default.post(`https://app.terraform.io/api/v2/workspaces/${id}/vars`, {
          data: {
            type: "vars",
            attributes: {
              key,
        ...
        }, tfOpts))).then(() => import_axios.default.post(`https://app.terraform.io/api/v2/runs`, {
          data: {
            attributes: {
    ...
    },
    {
      title: "Write .env",
      task: () => {
        return Promise.resolve(import_fs.default.writeFileSync(import_path.default.join(rootDirectory, ".env"), `API_URL=http://localhost:3003
  ...
  `));
    ...
    },
    {
      ...
      task: () => {
        console.log(import_chalk.default.blue("Manual steps to run:"));
        ...
        console.log(import_chalk.default.blue("- Copy "));
      }
    }
  ];
  const run = () => __async(void 0, null, function* () {
    ...
    for (const task of tasks) {
      ...
      const result = yield Promise.resolve(task.task).then((t) => t()).then(() => {
